...
public class ProductDetailFragment extends BaseFragment implements
	...
	private void initView() {
		...
		mProModel = (TextView) mView.findViewById(R.id.productDetailModel);
		mProCondition = (TextView) mView
				.findViewById(R.id.productDetailCondition);

		mProShortDescription = (WebView) mView
				.findViewById(R.id.productDetailDiscription);
		...
		mColorHorizontalView = (ColorHorizontalView) mView
				.findViewById(R.id.proDetailColorView);
		mProDetailActiveColor = (TextView) mView
				.findViewById(R.id.proDetailActiveColor);
		...
		mProgressDialog.setMessage(mHomeActivity
				.getString(R.string.detail_checking_color_stock));
	}
	...
	private void initListener() {
		...
		mColorHorizontalView
				.setOnTabReselectedListener(mOnTabReselectedListener);
	}

	private void checkColorAvailable(ColorDTO color) {
		if (mCheckColorStockAvailableTask == null
				|| mCheckColorStockAvailableTask.getStatus() == Status.FINISHED) {
			mCheckColorStockAvailableTask = new CheckColorStockAvailableTask(
					color);
	...
	private class CheckColorStockAvailableTask extends
			AsyncTask<Void, Void, StockDTO> {
		...
		protected StockDTO doInBackground(Void... params) {
			try {
				...
				return CommonModel.getInstance().getStock(
						temp.linkHref + "?ws_key=" + OrangeConfig.App_Key);
				// if(mStock=null)
				// return
				// CommonModel.getInstance().getColorStockAvailable(String.format(UrlRequest.PRODUCT_COLOR_ITEM_STOCK,
				// mProduct.id,temp.id_product_attribute));
			} catch (Exception e) {
	...
	private class LoadProductDetailTask extends
		...
		protected void onPostExecute(ProductDTO result) {
			...
			if (result != null) {
				...
				if (mProduct.listProductFeatures != null
						&& mProduct.listProductFeatures.size() > 0) {
					// String
					// dataSheet="<body><table border=\"1\" cellpadding=\"0\" cellspacing=\"0\" style=\"border-bottom: 1px solid #d6d4d4;width: 100%;background: white;\"><tbody>";
					String dataSheet = "<body><table style=\"background: white;\"><tbody>";
					for (ProductFeatureAndValueDTO item : mProduct.listProductFeatures) {
						dataSheet += "<tr style=\"border-top: 1px solid #d6d4d4; padding:0px;\"><td style=\"padding: 5px 20px 5px 0px;border-right: 1px solid #d6d4d4;width: 30%;font-weight: 900;color: #333333;\">"
								+ item.feature_title
								+ "</td><td style=\"text-align: left;font-weight: normal;vertical-align: middle;padding: 5px 20px 5px 20px;\">"
								+ item.feature_value + "</td></tr>";
					}
					dataSheet += "</tbody></table></body>";
					mDataSheet.loadData(dataSheet, "text/html", "utf-8");
				} else {
					mView.findViewById(R.id.dataSheetLabel).setVisibility(
							View.GONE);
					mDataSheet.setVisibility(View.GONE);
				}
				String shortDescriptions = "<body style='text-align: justify;font-size:17px;padding:0px;'>"
						+ mProduct.description_short + "</body>";
				String descriptions = "<body style='text-align: justify;font-size:17px;padding:0px;'>"
						+ mProduct.description + "</body>";
				// mProPriceDescription.setText(Html.fromHtml(descriptions));
				mProShortDescription.loadData(shortDescriptions, "text/html",
						"utf-8");
				...
				if (mProduct.associations.images.size() > 0) {
					mSilderAdapter = new ProductDetailImageSlider(
							mHomeActivity, mProduct.associations.images);
